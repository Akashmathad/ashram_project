...
function formatDateToYYYYMMDD(isoDateString: string) {
...
const UpdateStudent: FC<pageProps> = ({ params, student }) => {
  ...
  const form = useForm<StudentValidatorTypes>({
    ...
    defaultValues: {
      name: student.name,
      class: student.class,
      academicYear: formatDateToYYYYMMDD(
        new Date(student.academicYear).toISOString()
      ),
  ...
  const { mutate: updateStudent, isPending } = useMutation({
    mutationFn: async (values: StudentValidatorTypes) => {
      ...
      const { data } = await axios.post(
        `/api/student/update?studentId=${params.studentId}`,
        payload
      );
    ...
    onSuccess: () => {
      ...
      router.back();
      toast({
        title: 'Student updation successful',
      });
    },
  });
